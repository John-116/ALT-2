...
recList = {
  k: v
  for k, v in sorted(
    movie_scored_unsorted.items(), key=lambda x: x[1], reverse=True)
}

recList = {
  k: v
  for k, v in recList.items() if k not in user_movies_with_rating.keys()
}
...
for i in top_5_recommendations:
  print(i)
  overview = movies_raw["overview"][(list(
    movies_raw["original_title"]).index(i))]
  print(f"Overview:\n{overview}\n\n")
